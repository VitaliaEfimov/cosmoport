...
public class ShipServiceImpl implements ShipService {
    ...
    public Ship createShip(Ship ship) {
        if (ship.getName() == null
            ...
            throw new IncorrectInputException("One of Ship params is null");
    ...
    private void checkShipParams(Ship ship) {
        ...
        if (ship.getName() != null && (ship.getName().length() < 1 || ship.getName().length() > 50))
            throw new IncorrectInputException("Incorrect Ship.name");
        if (ship.getPlanet() != null && (ship.getPlanet().length() < 1 || ship.getPlanet().length() > 50))
            throw new IncorrectInputException("Incorrect Ship.planet");
        if (ship.getCrewSize() != null && (ship.getCrewSize() < 1 || ship.getCrewSize() > 9999))
            throw new IncorrectInputException("Incorrect Ship.crewSize");
        if (ship.getSpeed() != null && (ship.getSpeed() < 0.01D || ship.getSpeed() > 0.99D))
            throw new IncorrectInputException("Incorrect Ship.speed");
        if (ship.getProdDate() != null) {
            Calendar cal = Calendar.getInstance();
            cal.setTime(ship.getProdDate());
            if (cal.get(Calendar.YEAR) < 2800 || cal.get(Calendar.YEAR) > 3019)
                throw new IncorrectInputException("Incorrect Ship.date");
        }
    }
    ...
    public Long checkAndParseId(String id) {
        if (id == null || id.equals("") || id.equals("0"))
            throw new IncorrectInputException("Incorrect ID");
        try {
            Long longId = Long.parseLong(id);
            return longId;
        } catch (NumberFormatException e) {
            throw new IncorrectInputException("ID is not a number", e);
        }
    }
    ...
    private Double calculateRating(Ship ship) {
        ...
        BigDecimal raiting = new BigDecimal
